...
describe("send() — malformed ACK responses", () => {
  ...
  it("throws PARSE_FAILED with an IncompatibleCharsetError cause when the ACK bytes are not valid UTF-8", async () => {
    // A Latin-1 / Windows-1252 peer emits a lone 0xE9. The strict (fatal) UTF-8
    // decoder must surface this as MllpClientError(PARSE_FAILED), not leak a raw
    // TypeError — the error contract is "branch on code" — with the charset
    // package's IncompatibleCharsetError preserved on `cause`.
    ...
    expect((captured as MllpClientError).cause).toBeInstanceOf(
