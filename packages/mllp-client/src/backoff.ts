/**
 * Retry options + the connection-retry backoff delay (with jitter). Pure: no
 * state, no I/O. The connection machine (`../state.ts`) carries
 * {@link RetryOptions} and calls {@link backoffDelay} when scheduling the next
...
/** How the client retries a failed connection attempt (connect or reconnect). */
export interface RetryOptions {
  /**
   * Max retries after a failed attempt before giving up (→ `closed`). `0`
   * disables retry.
   */
...
/**
 * Retry disabled: a failed attempt (or a drop) goes straight to `closed`. The
