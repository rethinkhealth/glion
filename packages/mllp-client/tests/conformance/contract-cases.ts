...
export const contractCases: Readonly<Record<ContractCaseId, ContractCase>> = {
  ...
  "C14-reopen-after-close": {
    receiver: "acknowledges",
    rule: "connect() opens a fresh socket after close()",
    async run(socket) {
      await open(socket);
      await socket.close();
      let streams: MllpStreams;
      try {
        streams = await open(socket);
      } catch {
        return { read: "timeout", second: "rejected" };
      }
