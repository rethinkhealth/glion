...
export interface HarnessRequest {
  ...
  /** Attempts after a failed one, with no delay. Default: dial once. */
...
export interface HarnessOutcome {
  ...
  /** `connect` and `close:<code|null>`, in the order emitted. */
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    const client = new MllpClient({
      ...
      reconnect:
        body.reconnectAttempts === undefined
          ? false
          : { attempts: body.reconnectAttempts, delay: () => 0 },
