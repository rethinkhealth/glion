...
Every conformance scenario in §2.3 runs plain and, where the last column is "yes", over TLS with the self-signed pair from #616's `test/tls-fixtures.ts`. Workers TLS gets option-validation cases only, plus an optional manual check against a public TLS endpoint.
...
| --- | -------------------------------------- | ---------------------------------- | ------------------------------------------------------------------- |
| S1  | three sends on one connection          | acknowledges each frame            | `AA` × 3; events `connect`, `close:null`                            |
| S2  | acknowledgment split across two chunks | writes in two parts                | `AA`                                                                |
| S3  | silent receiver                        | accepts, never writes              | `SEND_TIMEOUT`; `close:SEND_TIMEOUT`; `state` `closed`              |
...
| S6  | nothing listening                      | closed port                        | `CONNECTION_FAILED`; event `close:CONNECTION_FAILED` only           |
| S7  | blackhole                              | TEST-NET-1, `connectTimeoutMs` 300 | `CONNECTION_TIMEOUT`                                                |
| S8  | application reject                     | answers `AE`                       | `AckApplicationError`; connection stays open; the next send is `AA` |
| S9  | `destroy()` during a send              | silent                             | in-flight send rejects `CLOSED`; events `connect`, `close:null`     |
| S10 | `close()` from `connected`             | accept                             | resolves; `state` `closed`; `close:null`                            |
...
| Integration | An adapter honours the `MllpSocket` contract, and the client behaves over it     | Real TCP, remote systems started by `setup.ts` | `packages/mllp-client/tests/integration/`                | `test:integration`; `test:bun`, `test:deno`, `test:cf` | Every push, one job per runtime                         |
