...
**Resolved 2026-09-07: (b).** (a) was built as a draft and measured rather than argued about. The mailbox actor preserved the public surface and passed the suite, but cost **+247 code lines (+32%)** against the shipped client, and every round of review removed machinery that turned out not to be load-bearing: a separate state module, a framed-connection wrapper, a `written` fact, a write `stage`, a phase-to-state translation, and per-send sequence numbers. Stripping the mailbox to a direct call left all 60 tests passing.

The prediction in ADR 0020 that source would _shrink_ assumed reconnect and a send queue would land inside the actor, where a new phase is a cheap table row. §2 and §3 of this plan move both above the client, so the actor was paying ceremony for a payoff it no longer collects. For a lockstep protocol with one message in flight and two interrupters (the deadline and `close()`), `async`/`await` already is the state machine.

What was kept from the draft, applied to the shipped client for **+17 code lines**: a total `decode()` returning `accepted | rejected | invalid` (removing a throw from the read path and moving the NAK table into the codec), `delivery: "unknown"` for a send interrupted by `close()`, the phase graph documented as a DAG with one back-edge, and three race tests. ADR 0020 needs rewriting to record the shipped design as the decision.
