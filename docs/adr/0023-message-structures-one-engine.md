...
2. **One compiler, one engine.** An internal compiler turns a structure into a Thompson NFA whose epsilon edges are ordered by priority and carry group open and close actions. Two functions run it:
   ...
   - `matchStructure(structure, names)` groups segments as a Pike VM: one thread per reading, in priority order, deduplicated by state so that the higher-priority thread wins, each carrying its group actions.
...
4. **The program is internal, compiled on first use.** Callers pass a message structure and never see its program: the compiled form is cached per structure object, and a bundled structure is compiled when it is loaded. Compiling all 2,136 structures takes 15 ms (about 7 µs each), once per structure and process; emitting the programs would add 2.4 MB to the package. An invalid structure throws from `runner` and `matchStructure`, and the build compiles every bundled structure, so a bad structure fails the build rather than a caller.

5. **Structures are open to callers.** A structure is plain data that the published JSON Schema describes, and one of the caller's own works wherever a bundled one does: `runner`, `matchStructure`, and the `definition` option of `@glion/lint-profile-events-segments-order`, which takes a structure or a function that chooses one per message.
...
- `Definition`, `TransitionMap`, `NFA`, `RunnerState`, and the `effects` API are removed from `@glion/profiles`; `runner` takes a `MessageStructure`, and so does the lint's `definition` option.
