...
- **Decode/parse failures never throw out of band.** `createContext` is the single total step — a non-UTF-8 payload or a parser throw yields an empty `Root` recorded on `ctx.error`. `handle()` then runs **one** try/catch: it re-throws `ctx.error` into that catch (alongside any handler/middleware throw), which routes to `#handleError` — the **same destination as a thrown handler error** — invoking the app's `onError` (it can build a NAK) or re-throwing to `serve()`. This mirrors Hono's `dispatch` (`try { compose } catch (err) → #handleError → errorHandler`) and Koa's top-level try/catch, rather than injecting a synthetic throwing step into the middleware chain. The ack **middleware** handles only in-chain (post-parse, application-level) throws; a non-UTF-8 frame reaches `onError` as before (#659/#660), now from the runtime-agnostic core.
