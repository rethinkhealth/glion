...
- **Every transport error layer is observable (no silent drops) through one channel.** Following the same model as Node's `server`/`socket`/`clientError` events but without three callbacks, `serve({ onError })` receives a discriminated `MllpErrorEvent` (`kind: "server" | "framing" | "connection"`). A startup/bind error rejects `server.listening`; a post-listen server error → `kind: "server"` (server keeps serving); a malformed/oversized MLLP envelope → `kind: "framing"` (connection torn down, **no NAK** per the MLLP spec — there's no message boundary, but `FRAME_TOO_LARGE` is a DoS signal worth seeing); an error that escaped the core → `kind: "connection"`; routine connection drops (ECONNRESET/timeout) stay silent. Framing events are PHI-safe — only the typed `code`, never the offending bytes. This closes the Mirth-style silent-drop footgun (NextGen Connect #3170). The server's per-layer _behaviour_ stays distinct; the single callback only unifies _observability_.
