...
- **Fixtures come from `fixtures/`** once two suites (or a suite and the canary) need them; single-suite shapes stay local with a comment. Profile-aware tree fixtures pin HL7 v2.5 — keep them on one version or cross-rule numbers stop being comparable.
