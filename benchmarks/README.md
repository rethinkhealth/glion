...
| `@glion/profiles`                                                         | `suites/profiles-runner.bench.ts`, `suites/profiles-structure.bench.ts`, `lab/message-structure-scaling.bench.ts`, `lab/profiles-cache.bench.ts` | order validation and grouping on the same ORU_R01 inputs; a 10 → 100,000-segment scaling sweep and a cache-layer comparison in the lab; cold/warm regression signal comes via the lint suite |
