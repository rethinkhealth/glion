...
Migration: pass a `string` (or `Root`) to `send()`; if you hold wire bytes, decode them to text at your I/O boundary (where charset / MSH-18 knowledge lives) and pass the `string`. A non-UTF-8 **inbound** ACK still surfaces `INVALID_RESPONSE` (ACKs are decoded as strict UTF-8 via `@glion/util-charset`).
