...
- `connect()`'s "which error" decision (`CLOSED` vs `ALREADY_CONNECTED`) is owned by the machine, which constructs and settles the typed error itself — the client no longer branches on the phase in its error handling.
