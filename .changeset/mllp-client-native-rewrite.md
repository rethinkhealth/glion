...
- **`delivery` on every error.** `not-sent` means nothing reached the wire and sending again is safe; `unknown` means the message may have been received and should be sent again only when it is safe to repeat. Connect, option, state, and message failures are `not-sent`; a send timeout, a dropped connection, and an unusable reply are `unknown`.
