...
- Add `@glion/lint-charset` — an `hl7v2-lint` rule that checks every `MSH-18` repetition against an allow-list (default `["UNICODE UTF-8", "ASCII", "ISO IR6"]`, the UTF-8-compatible HL7 table 0211 codes), matched case-insensitively. An absent or empty `MSH-18` passes (the spec default is ASCII) unless the `required` option is set; the allow-list is configurable via the `allow` option. The rule reports only its own diagnostic and ignores non-message nodes.
- Add `@glion/mllp-charset` with `charsetMiddleware()` — an MLLP middleware that runs the pipeline (`await ctx.tree()`), and on a fatal charset diagnostic throws `AckApplicationReject`. Registered inside `ackMiddleware`, that becomes an `AR` NAK located at the offending `MSH-18` node (ERR-2) with the reason as diagnostic information (ERR-7); this is the rule-reports / server-decides split — the rule never aborts the pipeline itself.
- Extend `@glion/ack` so `AckException` can build a richer ERR segment: new optional `node`/`ancestors` (ERR-2 is derived from the responsible node's position via `@glion/util-query`'s `format`) and `diagnosticInformation` (ERR-7). The ERR segment stays byte-identical when none are supplied, so existing acknowledgments are unchanged (#665).
