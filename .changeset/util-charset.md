...
- Change the MLLP server to decode payloads via `decodeBytes`; a non-UTF-8 message now surfaces through `onError` (as an `CharsetError`) instead of being decoded to U+FFFD and acknowledged as valid
- Change the MLLP client to decode ACKs via `decodeBytes`; a non-UTF-8 ACK now rejects with `MllpErrorCode.PARSE_FAILED`, carrying the `CharsetError` on `cause`
