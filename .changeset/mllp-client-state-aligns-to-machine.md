...
- `MllpClientState` is now `idle | connecting | connected | backingOff | closed` (the machine's `ConnectionPhase`).
...
This drops the `onWire`/`closing` bookkeeping so the public state can never drift from the actual lifecycle. `backingOff` is unreachable until retry is enabled.
